...
from ibwt.paullaplonesite.setuphandlers import setup_catalog
from ibwt.paullaplonesite import upgrades


ENABLED = 1
PRODUCT = 'ibwt.paullaplonesite'
log = upgrades.log
move = upgrades.move
...
class FileIbwtPaullaplonesiteMigrator(BlobMigrator):
    src_portal_type = 'FileIbwtPaullaplonesite'
    src_meta_type = 'FileIbwtPaullaplonesite'
    dst_portal_type = 'FileIbwtPaullaplonesite'
    dst_meta_type = 'FileIbwtPaullaplonesite'


class IbwtPaullaplonesite_migrate(grok.View):
    ...
    def custom_hook_blob(self):
        self.migrate_fileIbwtPaullaplonesite_to_blob()
    ...
    def custom_hook_after_plone_upgrade(self):
        ...
        ps.runImportStepFromProfile(
            'profile-IbwtPaullaplonesite.skin:default', "skins", run_dependencies=False)
    ...
    @onlyonce
    def migrate_fileIbwtPaullaplonesite_to_blob(self):
        upgrades.log('Migrating FileIbwtPaullaplonesite to blob')
        walker = CustomQueryWalker(self.context, FileIbwtPaullaplonesiteMigrator, full_transaction=True)
        walker.go()
        upgrades.commit(self.context)
    ...
    def log_files(self, idx='1'):
        ptypes = ['Image', 'FileIbwtPaullaplonesite', 'File',
