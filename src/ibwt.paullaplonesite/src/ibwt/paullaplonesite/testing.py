...
class IbwtPaullaplonesiteLayer(PloneSandboxLayer):

    defaultBases = (PLONE_FIXTURE, )
    """Layer to setup the paullaplonesite site"""
    class Session(dict):
        def set(self, key, value):
            self[key] = value

    def setUpZope(self, app, configurationContext):
        """Set up the additional products required for the ibwt) site paullaplonesite.
        ...
        # -----------------------------------------------------------------------
        import ibwt.paullaplonesite
        self.loadZCML('configure.zcml', package=ibwt.paullaplonesite)
    ...
    def setUpPloneSite(self, portal):
        ...
        self.applyProfile(portal, 'ibwt.paullaplonesite:default')


IBWT_PAULLAPLONESITE_FIXTURE = IbwtPaullaplonesiteLayer(name='IbwtPaullaplonesite:Fixture')


class LayerMixin(Layer):
    defaultBases = (IBWT_PAULLAPLONESITE_FIXTURE,)
...
IBWT_PAULLAPLONESITE_SIMPLE = SimpleLayer(name='IbwtPaullaplonesite:Simple')
IBWT_PAULLAPLONESITE_INTEGRATION_TESTING = IntegrationTesting(name="IbwtPaullaplonesite:Integration")
IBWT_PAULLAPLONESITE_FUNCTIONAL_TESTING = FunctionalTesting(name="IbwtPaullaplonesite:Functional")
IBWT_PAULLAPLONESITE_SELENIUM_TESTING = FunctionalTesting(
    bases=(SELENIUM_TESTING, IBWT_PAULLAPLONESITE_FUNCTIONAL_TESTING,),
    name="IbwtPaullaplonesite:Selenium")
...
class Browser(z2.Browser):  # pragma: no cover
    ...
    def new(cls, url=None, user=None, passwd=None, headers=None, login=False):
        ...
        app = IBWT_PAULLAPLONESITE_FUNCTIONAL_TESTING['app']
        portal = IBWT_PAULLAPLONESITE_FUNCTIONAL_TESTING['portal']
        if not url:
